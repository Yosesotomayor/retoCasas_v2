...
rstate = 42
base_models = {
    "lasso": Lasso(alpha=0.0005, max_iter=5000, random_state=rstate),
    "rf": RandomForestRegressor(n_estimators=600, max_features="sqrt", random_state=rstate, n_jobs=-1),
...
M = np.column_stack([oof_preds["lasso"], oof_preds["rf"], oof_preds["lgbm"]])
y_true = y.values
...
for w1 in np.arange(0, 1 + 1e-9, grid_step):
    for w2 in np.arange(0, 1 - w1 + 1e-9, grid_step):
        ...
        if rmse < best["rmse"]:
            best = {"weights": w.tolist(), "rmse": float(rmse), "r2": float(r2)}

opt_weights = {"lasso": best["weights"][0], "rf": best["weights"][1], "lgbm": best["weights"][2]}
...
out_dir = "datascience/models/3pipemodelFINAL"
os.makedirs(out_dir, exist_ok=True)

joblib.dump(final_pipes["lasso"], os.path.join(out_dir, "final_pipe_lasso.joblib"))
joblib.dump(final_pipes["rf"],    os.path.join(out_dir, "final_pipe_rf.joblib"))
...
preds = np.column_stack([
    final_pipes["lasso"].predict(df_test.drop(["Id"], axis=1)),
    final_pipes["rf"].predict(df_test.drop(["Id"], axis=1)),
    final_pipes["lgbm"].predict(df_test.drop(["Id"], axis=1))
])
w_vec = np.array([opt_weights["lasso"], opt_weights["rf"], opt_weights["lgbm"]], dtype=float)
pred_log = preds @ w_vec
pred_orig = np.expm1(pred_log)

sub_dir = "data/housing_submissions/3modelpipelineRAW"
