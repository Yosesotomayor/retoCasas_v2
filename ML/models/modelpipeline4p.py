...
base_models = {
    ...
    "histboost": HistGradientBoostingRegressor(max_iter=3000, learning_rate=0.03, random_state=rstate),
    "catboost": CatBoostRegressor(n_estimators=3000, learning_rate=0.03, random_state=rstate),
...
M = np.column_stack([oof_preds["elasticnet"], oof_preds["histboost"], oof_preds["catboost"], oof_preds["lgbm"]])
y_true = y.values

for w1 in np.arange(0, 1 + 1e-9, grid_step):
    for w2 in np.arange(0, 1 - w1 + 1e-9, grid_step):
        for w3 in np.arange(0, 1 - w1 - w2 + 1e-9, grid_step):
            w4 = 1 - w3 - w2 - w1
            y_pred = w1 * oof_preds["elasticnet"] + w2 * oof_preds["histboost"] + w3 * oof_preds["catboost"] + w4 * oof_preds["lgbm"]
            rmse = np.sqrt(mean_squared_error(y_true, y_pred))
            r2 = r2_score(y_true, y_pred)
            if rmse < best["rmse"]:
                best["weights"] = (w1, w2, w3, w4)
                best["rmse"] = rmse
                best["r2"] = r2

opt_weights = {"elasticnet": best["weights"][0], "histboost": best["weights"][1], "catboost": best["weights"][2], "lgbm": best["weights"][3]}
...
out_dir = "datascience/models/pipemodel4p"
os.makedirs(out_dir, exist_ok=True)

joblib.dump(final_pipes["elasticnet"], os.path.join(out_dir, "final_pipe_elasticnet.joblib"))
joblib.dump(final_pipes["histboost"],    os.path.join(out_dir, "final_pipe_histboost.joblib"))
joblib.dump(final_pipes["catboost"], os.path.join(out_dir, "final_pipe_catboost.joblib"))
...
preds = np.column_stack([
    ...
    final_pipes["histboost"].predict(df_test.drop(["Id"], axis=1)),
    final_pipes["catboost"].predict(df_test.drop(["Id"], axis=1)),
    final_pipes["lgbm"].predict(df_test.drop(["Id"], axis=1))
])

w_vec = np.array([opt_weights["elasticnet"], opt_weights["histboost"], opt_weights["catboost"], opt_weights["lgbm"]], dtype=float)
pred_log = preds @ w_vec
pred_orig = np.expm1(pred_log)

sub_dir = "data/housing_submissions/modelpipeline4p"
os.makedirs(sub_dir, exist_ok=True)
submission = pd.DataFrame({"Id": df_test["Id"], "SalePrice": pred_orig})
submission_path = os.path.join(sub_dir, "submission4p.csv")
